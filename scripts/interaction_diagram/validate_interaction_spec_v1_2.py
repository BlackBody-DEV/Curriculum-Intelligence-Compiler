"""Version-dispatching, fail-closed interaction specification validator v1.2."""
...
import jsonschema
from .formulas_v1_2 import FORMULA_METADATA, FormulaError, evaluate_formula
...
from .validate_interaction_spec_v1_1 import validate_interaction_spec as validate_v11
...
def security_rejections(spec):
    ...
    for constraint in list(spec.get('geometric_constraints',[]))+list(spec.get('mathematical_constraints',[])):
        ...
        if model_id is not None and model_id not in CONSTRAINT_MODELS: out.append(f'unsupported constraint_model_id: {model_id!r}')
    for label in spec.get('visible_labels',[]):
        vocabulary_id=label.get('display_vocabulary_id')
        if vocabulary_id is not None and vocabulary_id not in DISPLAY_VOCABULARIES: out.append(f'unsupported display_vocabulary_id: {vocabulary_id!r}')
...
def validate_interaction_spec(spec,*,procedure_registry=None):
    ...
    if isinstance(spec,dict) and spec.get('schema_version')=='1.1.0': return validate_v11(spec,procedure_registry=procedure_registry)
