from tools.course_compiler_demo.production_question_packs.mechanics import build_bank


def test_mechanics_real_locked_bank_and_coverage():
    ...
    assert all("path test" not in x["prompt"].lower() for x in bank.candidates)


def test_mechanics_is_deterministic_and_nonfixture():
    a = build_bank()[0]
    b = build_bank()[0]
    assert a.to_json() == b.to_json()
    assert all(x["safety"]["synthetic_fixture"] is False and x["safety"]["production_candidate"] is True for x in a.candidates)
    assert all(x["findings"] and x["reviewer"] == "independent_mechanics_artifact_reviewer" for x in a.reviews)


def test_mechanics_validator_rejects_missing_units_and_axes():
    from tools.course_compiler_demo.production_question_packs.mechanics.bank import artifact_reviewer, mechanics_validator, _family
    ...
    assert not mechanics_validator(candidate, derivation, derivation.normalized_answer).passed

    course = build_physics_engineering_course_catalog()["courses"]["MECHANICS"]
    with pytest.raises(ValueError):
        artifact_reviewer(tuple(_family(i, course) for i in range(10)), {}, "MECHANICS_PRODUCTION_00", "FAMILY")
