"""Physics 126 Waves and Optics completion: a third locked bank of 100 questions."""
...
from .bank import _family, artifact_reviewer, build_bank, waves_validator
...
def _completion_family(index: int, course: dict) -> ProductionFamily:
    ...
    def completion_parameters(candidate_index: int, builder=base.parameter_builder) -> dict:
        ...
        parameters["a"] += 400.0
        parameters["frequency"] += 40.0
        parameters["length"] += 20.0
        parameters["distance"] += 40.0
        if index == 3:
            parameters["distance"] = parameters["a"] + float((candidate_index % 8) + 10)
        if index != 6:
            parameters["b"] += 10.0
        return parameters
...
def build_completion_bank():
    ...
    bank = replace(bank, bank_id="bank:WAVES_AND_OPTICS:physics-126-completion:v1")
    summary = replace(summary, summary_id="summary:WAVES_AND_OPTICS:physics-126-completion:v1")
    return bank, summary
