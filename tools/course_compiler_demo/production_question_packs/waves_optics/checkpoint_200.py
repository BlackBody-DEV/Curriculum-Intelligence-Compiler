"""Physics 124 Waves and Optics checkpoint: a second locked bank of 100 questions."""
...
def _checkpoint_family(index: int, course: dict) -> ProductionFamily:
    ...
    def checkpoint_parameters(candidate_index: int, builder=base.parameter_builder) -> dict:
        ...
        if index == 3:
            parameters["distance"] = parameters["a"] + float((candidate_index % 8) + 2)
        if index != 6:
            parameters["b"] += 5.0
        return parameters
...
def build_checkpoint_bank():
    ...
    bank = replace(bank, bank_id="bank:WAVES_AND_OPTICS:physics-124-checkpoint:v1")
    summary = replace(summary, summary_id="summary:WAVES_AND_OPTICS:physics-124-checkpoint:v1")
    return bank, summary
