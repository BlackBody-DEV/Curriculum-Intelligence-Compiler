"""Math 127 Precalculus completion: 100 questions taking the course to 300."""
...
COURSE_ID="PRE_CALCULUS"; FIRST_NEW_VARIANT=40

def build_completion_300_bank()->tuple[tuple[dict[str,Any],...],dict[str,Any]]:
    catalog=discover_course_catalog(); course=catalog["new"][COURSE_ID]; orchestration=discover_generation_recipe_runtime(catalog["new"]); pilot=compile_course_pilot(course,orchestration); bank_100,summary_100=build_checkpoint_bank(); bank_200,summary_200=build_checkpoint_200_bank()
    if pilot["validated"]!=25 or pilot["locked"]!=25 or summary_100["after"]!=100 or summary_200["after"]!=200: raise ValueError("authoritative 200-question starting inventory is unavailable")
    ...
    seen_ids={q["candidate_id"] for q in pilot["questions"]}|{q["question_id"] for q in bank_100}|{q["question_id"] for q in bank_200}; seen_semantic={q["semantic_fingerprint"] for q in pilot["questions"]}|{q["semantic_fingerprint"] for q in bank_100}|{q["semantic_fingerprint"] for q in bank_200}; seen_prompts={_prompt_fingerprint(q["prompt"]) for q in pilot["questions"]}|{q["prompt_fingerprint"] for q in bank_100}|{q["prompt_fingerprint"] for q in bank_200}
    ...
    for source in sorted(accepted,key=lambda item:item["recipe"].recipe_id):
        ...
        for variant in range(FIRST_NEW_VARIANT,FIRST_NEW_VARIANT+1000):
            ...
            context=GenerationContextV1(binding,source["topic"]["title"],source["skill"]["title"],tuple(source["procedure"]["steps"]),f"math127:{COURSE_ID}:{binding.family_id}",variant); result=orchestration["runtime"].generate(recipe.recipe_id,context,family); contract=recipe.build_contract(dict(result.parameters)); semantic=result.content_sha256; prompt_fp=_prompt_fingerprint(result.prompt); question_id=f"production-question:{COURSE_ID.lower()}:math127:{recipe.recipe_id.rsplit(':',1)[-1].lower()}:{variant:02d}"
    ...
    families=Counter(q["generation_family_id"] for q in rows); structures=Counter(q["structural_fingerprint"] for q in rows); topics=Counter(q["topic_id"] for q in rows); skills=Counter(q["micro_skill_id"] for q in rows); procedures=Counter(q["procedure_id"] for q in rows); modes=Counter(q["coverage_mode"] for q in rows)
    if len(families)<5 or max(families.values())>25 or max(structures.values())>25 or len(topics)<5 or len(skills)<5 or len(procedures)<5 or len(modes)!=2: raise ValueError("coverage or diversity gate failed")
    bank_hash=hashlib.sha256(json.dumps(rows,sort_keys=True,separators=(",",":"),default=list).encode()).hexdigest()
    return tuple(rows),{"course_id":COURSE_ID,"before":200,"added":100,"after":300,"generated":100,"validated":100,"locked":100,"identity_duplicates":0,"prompt_duplicates":0,"fingerprint_duplicates":0,"exact_duplicates":0,"family_distribution":dict(sorted(families.items())),"topic_count":len(topics),"micro_skill_count":len(skills),"procedure_count":len(procedures),"coverage_distribution":dict(sorted(modes.items())),"difficulty_levels":sorted({q["difficulty"] for q in rows}),"maximum_family_share":max(families.values())/100,"maximum_structural_share":max(structures.values())/100,"excessive_parameter_only_repetition":False,"unsupported_answer_shapes":0,"bank_sha256":bank_hash,"status":"PASS"}
