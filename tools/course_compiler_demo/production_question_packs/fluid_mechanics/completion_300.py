...
def _g(i,c):
 b=_f(i,c)
 def p(n):x=b.parameter_builder(n);x.update(a=x["a"]+200,b=x["b"]+10,rho=x["rho"]+200,velocity=x["velocity"]+10,area=x["area"]+.2,length=x["length"]+20,diameter=x["diameter"]+.1,viscosity=x["viscosity"]+.001,height=x["height"]+5);return x
 return replace(b,parameter_builder=p)
def build_completion_bank():
 prior=(build_bank()[0],build_checkpoint_bank()[0]);pack=build_physics_engineering_course_catalog();fs=tuple(_g(i,pack["courses"]["FLUID_MECHANICS"]) for i in range(10));seen={}
 for b in prior:
  for x in b.candidates:duplicate_record(ProductionQuestionCandidateV1(**x),seen)
 ev=({"evidence_id":"FLUID_MECHANICS:CATALOG","source_identity":pack["pack_id"],"source_hash":pack["deterministic_sha256"]},);b,s=produce_course_bank("FLUID_MECHANICS",pack["pack_id"],pack["deterministic_sha256"],ev,fs,duplicate_analyzer=lambda c,_:duplicate_record(c,seen),validator=validator);return replace(b,bank_id="bank:FLUID_MECHANICS:completion-300:v1"),s
