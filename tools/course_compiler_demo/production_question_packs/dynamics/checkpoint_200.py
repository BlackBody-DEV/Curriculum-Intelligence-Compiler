...
def _checkpoint_family(index, course):
    ...
    def parameters(n,builder=base.parameter_builder):
        row=builder(n); row["a"]+=200.0; row["b"]+=20.0; row["time"]+=10.0; row["mass"]+=20.0; row["speed"]+=20.0; row["radius"]+=10.0; return row
    return replace(base,parameter_builder=parameters)


def build_checkpoint_bank():
    existing=build_bank()[0]; pack=build_physics_engineering_course_catalog(); validate_physics_engineering_course_catalog(pack); course=pack["courses"]["DYNAMICS"]; families=tuple(_checkpoint_family(i,course) for i in range(10)); evidence=({"evidence_id":"DYNAMICS:COURSE_CATALOG","source_identity":pack["pack_id"],"source_hash":pack["deterministic_sha256"],"access":"READ_ONLY_REFERENCE"},); seen={}
    ...
    return replace(bank,bank_id="bank:DYNAMICS:checkpoint-200:v1"),replace(summary,summary_id="summary:DYNAMICS:checkpoint-200:v1")
...
def audit_checkpoint():
    ...
    return {"course_id":"DYNAMICS","before":100,"added":100,"after":len(set(ids)),"duplicate_identities":len(ids)-len(set(ids)),"exact_prompt_duplicates":len(prompts)-len(set(prompts)),"fingerprint_duplicates":len(fingerprints)-len(set(fingerprints)),"exact_record_duplicates":len(records)-len(set(records)),"validated":summary.validated,"status":"PASS" if passed else "FAIL"}
