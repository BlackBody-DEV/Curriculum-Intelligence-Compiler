"""Course-local Linear Algebra production bank builders."""
...
def _matrix_ok(engine:str,value:Any,derived:Any)->bool:
 if engine!="matrix":return value==derived
 return isinstance(value,list) and len(value)>0 and all(isinstance(r,list) and len(r)==len(value[0]) for r in value) and value==derived
def _build(prior:list[dict[str,Any]],before:int,added:int,start:int,label:str)->tuple[tuple[dict[str,Any],...],dict[str,Any]]:
 ...
 for src in sorted(accepted,key=lambda x:x["recipe"].recipe_id):
  ...
  for v in range(start,start+1000):
   ...
   ids.add(qid);sem.add(sid);prompts.add(pfp);valid={"operation_statuses":statuses,"answer_contract_valid":contract.engine_type==b.engine_type,"procedure_compatible":b.procedure_id==src["procedure"]["procedure_id"],"prompt_determinate":bool(z.prompt.strip()) and "{{" not in z.prompt,"failure_signals_valid":bool(family.get("failure_signals")),"matrix_dimension_and_consistency_valid":_matrix_ok(b.engine_type,z.normalized_answer,z.derivation_result.value)}
   rows.append({"question_id":qid,"course_id":COURSE_ID,"unit_id":src["topic"]["unit_id"],"topic_id":b.topic_id,"micro_skill_id":b.micro_skill_id,"procedure_id":b.procedure_id,"generation_family_id":b.family_id,"recipe_id":r.recipe_id,"difficulty":diffs[n%3],"coverage_mode":"MULTI_STEP" if contract.engine_type=="matrix" else "DIRECT_APPLICATION","prompt":z.prompt,"normalized_answer":z.normalized_answer,"answer_contract":contract.to_dict(),"grading_rule":{"engine_type":b.engine_type,"unsupported_shapes_fail_closed":True},"failure_signals":tuple(family.get("failure_signals",())),"provenance":{"provider":src["provider"],"deterministic_seed":ctx.seed,"variant_index":v,"content_sha256":sid},"semantic_fingerprint":sid,"prompt_fingerprint":pfp,"structural_fingerprint":_sf(z.prompt),"production_status":"LOCKED_PRODUCTION_VALIDATED","duplicate_status":"UNIQUE","independent_derivation":z.derivation_result.to_dict(),"validation":valid});n+=1
  if n!=each:raise ValueError("unique allocation failed")
 fam=Counter(q["generation_family_id"] for q in rows);struct=Counter(q["structural_fingerprint"] for q in rows);modes={q["coverage_mode"] for q in rows};gates=("answer_contract_valid","procedure_compatible","prompt_determinate","failure_signals_valid","matrix_dimension_and_consistency_valid")
