"""Physics 111 E&M checkpoint: a third locked bank of 100 questions."""
...
from .bank import _family, artifact_reviewer, build_bank, em_validator, validate_conventions
from .checkpoint_200 import build_checkpoint_bank as build_previous_checkpoint_bank


CHECKPOINT_PARAMETER_DELTA = 2000.0
...
def build_checkpoint_bank():
    ...
    previous_bank, _ = build_previous_checkpoint_bank()
    pack = build_physics_engineering_reference_pack()
    validate_physics_engineering_reference_pack(pack)
    course = pack["courses"]["ELECTRICITY_AND_MAGNETISM"]
    families = tuple(_checkpoint_family(index, course) for index in range(12))
    evidence = ({
        "evidence_id": "EM:REFERENCE_PACK",
        "source_identity": pack["pack_id"],
        "source_hash": pack["deterministic_sha256"],
        "access": "NONCANONICAL_REFERENCE_PACK",
    },)

    seen: dict[str, str] = {}
    for payload in tuple(existing_bank.candidates) + tuple(previous_bank.candidates):
    ...
    def validator(candidate, derivation, generator_answer):
        result = em_validator(candidate, derivation, generator_answer)
    ...
    validate_conventions(bank)
    bank = replace(bank, bank_id="bank:ELECTRICITY_AND_MAGNETISM:physics-111-checkpoint-300:v1")
    summary = replace(summary, summary_id="summary:ELECTRICITY_AND_MAGNETISM:physics-111-checkpoint-300:v1")
    return bank, summary
...
def audit_checkpoint() -> dict:
    ...
    prior_bank, _ = build_previous_checkpoint_bank()
    added, summary = build_checkpoint_bank()
    inventory = build_checkpoint_inventory(added)
    all_candidates = tuple(existing.candidates) + tuple(prior_bank.candidates) + tuple(added.candidates)
    existing_ids = {q["candidate_id"] for q in existing.candidates}
    prior_ids = {q["candidate_id"] for q in prior_bank.candidates}
    ...
    fingerprints = [row["fingerprint"] for row in existing.duplicates] + [row["fingerprint"] for row in prior_bank.duplicates] + [row["fingerprint"] for row in added.duplicates]
    passed = (
        len(existing_ids) == 100
        and len(prior_ids) == 100
        and len(added_ids) == 100
        and len(existing_ids | prior_ids | added_ids) == 300
        and not (existing_ids & prior_ids)
        and not (existing_ids & added_ids)
        and not (prior_ids & added_ids)
        and len(prompt_hashes) == len(set(prompt_hashes)) == 300
        and len(fingerprints) == len(set(fingerprints)) == 300
    ...
    return {
        ...
        "before": len(existing_ids | prior_ids),
        "added": len(added_ids),
        "after": len(existing_ids | prior_ids | added_ids),
        "identity_overlap": len((existing_ids | prior_ids) & added_ids),
