"""Math 119 Trigonometry checkpoint: 75 questions completing the first 100."""
...
def build_checkpoint_bank() -> tuple[tuple[dict[str, Any], ...], dict[str, Any]]:
    ...
    for row in sorted(accepted, key=lambda item: item["recipe"].recipe_id):
        ...
        for variant in range(FIRST_NEW_VARIANT, LAST_NEW_VARIANT + 1):
            context = GenerationContextV1(binding, row["topic"]["title"], row["skill"]["title"],
                tuple(row["procedure"]["steps"]), f"math119:{COURSE_ID}:{binding.family_id}", variant)
    ...
    family_distribution = Counter(row["generation_family_id"] for row in rows)
    structural_distribution = Counter(row["structural_fingerprint"] for row in rows)
    if max(family_distribution.values()) > len(rows) * .25 or max(structural_distribution.values()) > len(rows) * .25:
        raise ValueError("excessive family allocation or parameter-only repetition")
