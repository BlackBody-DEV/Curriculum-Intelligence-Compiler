"""Math 122 Trigonometry completion: 100 questions taking the course to 300."""
...
def build_completion_300_bank() -> tuple[tuple[dict[str, Any], ...], dict[str, Any]]:
    ...
    pilot=compile_course_pilot(course,orchestration); checkpoint_100,summary_100=build_checkpoint_bank(); checkpoint_200,summary_200=build_checkpoint_200_bank()
    if pilot["validated"]!=25 or summary_100["after"]!=100 or summary_200["after"]!=200: raise ValueError("authoritative 200-question starting inventory is unavailable")
    ...
    seen_ids=({q["candidate_id"] for q in pilot["questions"]}|{q["question_id"] for q in checkpoint_100}|{q["question_id"] for q in checkpoint_200})
    seen_semantic=({q["semantic_fingerprint"] for q in pilot["questions"]}|{q["semantic_fingerprint"] for q in checkpoint_100}|{q["semantic_fingerprint"] for q in checkpoint_200})
    seen_prompts=({_prompt_fingerprint(q["prompt"]) for q in pilot["questions"]}|{q["prompt_fingerprint"] for q in checkpoint_100}|{q["prompt_fingerprint"] for q in checkpoint_200})
    ...
    for source in sorted(accepted,key=lambda item:item["recipe"].recipe_id):
        ...
        for variant in range(FIRST_NEW_VARIANT,FIRST_NEW_VARIANT+1000):
            ...
            context=GenerationContextV1(binding,source["topic"]["title"],source["skill"]["title"],tuple(source["procedure"]["steps"]),f"math122:{COURSE_ID}:{binding.family_id}",variant)
            ...
            if question_id in seen_ids or semantic_fingerprint in seen_semantic or prompt_fingerprint in seen_prompts: continue
            ...
            validation={"operation_statuses":statuses,"normalization_result":result.normalization_result.to_dict(),"derivation_result":result.derivation_result.to_dict(),
                "grading_result":result.grading_result.to_dict(),"procedure_compatible":binding.procedure_id==source["procedure"]["procedure_id"],
                "prompt_determinate":bool(result.prompt.strip()) and "{{" not in result.prompt,"failure_signals_valid":bool(family.get("failure_signals")),
            ...
            rows.append({"question_id":question_id,"course_id":COURSE_ID,"unit_id":source["topic"]["unit_id"],"topic_id":binding.topic_id,
                ...
                "recipe_id":recipe.recipe_id,"difficulty":difficulties[family_added%3],"prompt":result.prompt,"normalized_answer":result.normalized_answer,
                "answer_contract":contract.to_dict(),"grading_rule":{"engine_type":binding.engine_type,"normalization_required":True,
                    "independent_derivation_required":True,"unsupported_shapes_fail_closed":True},"failure_signals":tuple(family.get("failure_signals",())),
                "provenance":{"provider":source["provider"],"recipe_version":recipe.recipe_version,"deterministic_seed":context.seed,
                    "variant_index":variant,"content_sha256":result.content_sha256},"semantic_fingerprint":semantic_fingerprint,
                "prompt_fingerprint":prompt_fingerprint,"structural_fingerprint":_structural_fingerprint(result.prompt),"duplicate_status":"UNIQUE",
    ...
    return tuple(rows),{"course_id":COURSE_ID,"before":200,"added":100,"after":300,"generated":100,"validated":100,"locked":100,
        "identity_duplicates":0,"prompt_duplicates":0,"fingerprint_duplicates":0,"exact_duplicates":0,
...
def audit_completion_300() -> dict[str, Any]:
    ...
    return {**summary,"deterministic_replay":summary["bank_sha256"]==replay_summary["bank_sha256"]
        and [q["question_id"] for q in rows]==[q["question_id"] for q in replay],"metadata_complete":all(q["semantic_fingerprint"]
        and q["prompt_fingerprint"] and q["production_status"]=="LOCKED_PRODUCTION_VALIDATED" for q in rows)}
