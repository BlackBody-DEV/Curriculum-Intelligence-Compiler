"""Task 132 course-local production builders."""
...
COURSE_ID="ENGINEERING_ANALYSIS"
...
def shape_valid(engine:str,value:Any)->bool:
 return engine!="matrix" or isinstance(value,list) and bool(value) and all(isinstance(r,list) and len(r)==len(value[0]) for r in value)
def finite_valid(value:Any)->bool:return not isinstance(value,float) or math.isfinite(value)
def build_additions(course_id:str,prior:list[dict[str,Any]],before:int,added:int,start:int,label:str):
 ...
 for src in sorted(accepted,key=lambda x:x["recipe"].recipe_id):
  ...
  for variant in range(start,start+1000):
   ...
   ids.add(qid);sem.add(sid);prompts.add(pfp);units_declared=bool(contract_dict.get("units"));validation={"operation_statuses":statuses,"answer_contract_valid":contract.engine_type==binding.engine_type,"procedure_compatible":binding.procedure_id==src["procedure"]["procedure_id"],"prompt_determinate":bool(result.prompt.strip()) and "{{" not in result.prompt,"failure_signals_valid":bool(family.get("failure_signals")),"equivalence_valid":result.derivation_result.value==result.normalized_answer,"matrix_system_dimension_valid":shape_valid(binding.engine_type,result.normalized_answer),"dimensional_units":{"applicable":units_declared,"valid":not units_declared},"convergence_error_valid":finite_valid(result.normalized_answer)}
   rows.append({"question_id":qid,"course_id":course_id,"unit_id":src["topic"]["unit_id"],"topic_id":binding.topic_id,"micro_skill_id":binding.micro_skill_id,"procedure_id":binding.procedure_id,"generation_family_id":binding.family_id,"recipe_id":recipe.recipe_id,"difficulty":diffs[n%3],"coverage_mode":"MULTI_STEP" if binding.engine_type in {"matrix","symbolic_expression"} else "DIRECT_APPLICATION","prompt":result.prompt,"normalized_answer":result.normalized_answer,"answer_contract":contract_dict,"grading_rule":{"engine_type":binding.engine_type,"unsupported_shapes_fail_closed":True},"failure_signals":tuple(family.get("failure_signals",())),"provenance":{"provider":src["provider"],"deterministic_seed":ctx.seed,"variant_index":variant,"content_sha256":sid},"semantic_fingerprint":sid,"prompt_fingerprint":pfp,"structural_fingerprint":structural_fingerprint(result.prompt),"production_status":"LOCKED_PRODUCTION_VALIDATED","duplicate_status":"UNIQUE","independent_derivation":result.derivation_result.to_dict(),"validation":validation});n+=1
  if n!=each:raise ValueError("unique allocation failed")
 gates=("answer_contract_valid","procedure_compatible","prompt_determinate","failure_signals_valid","equivalence_valid","matrix_system_dimension_valid","convergence_error_valid");families=Counter(q["generation_family_id"] for q in rows);structures=Counter(q["structural_fingerprint"] for q in rows);modes={q["coverage_mode"] for q in rows}
 if len(rows)!=added or len({q["question_id"] for q in rows})!=added or len({q["semantic_fingerprint"] for q in rows})!=added or len({q["prompt_fingerprint"] for q in rows})!=added or not all(all(q["validation"][g] for g in gates) and q["validation"]["dimensional_units"]["valid"] for q in rows):raise ValueError("validation failed")
...
def build_checkpoint_bank():
 ...
 return build_additions(COURSE_ID,list(p["questions"]),25,75,5,"math132c100")
