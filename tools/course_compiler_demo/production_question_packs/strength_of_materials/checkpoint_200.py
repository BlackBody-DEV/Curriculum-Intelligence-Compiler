...
from .bank import _family,build_bank,reviewer,strength_validator
def _checkpoint_family(i,course):
 base=_family(i,course)
 def params(n,builder=base.parameter_builder):
  x=builder(n);x["a"]+=200;x["b"]+=10;x["pressure"]+=1;x["radius"]+=200;x["thickness"]+=10;x["length"]+=2000;x["modulus"]+=20;x["inertia"]+=20000;x["area"]+=200;return x
 return replace(base,parameter_builder=params)
def build_checkpoint_bank():
 old=build_bank()[0];pack=build_physics_engineering_course_catalog();validate_physics_engineering_course_catalog(pack);course=pack["courses"]["STRENGTH_OF_MATERIALS"];families=tuple(_checkpoint_family(i,course) for i in range(10));evidence=({"evidence_id":"STRENGTH_OF_MATERIALS:COURSE_CATALOG","source_identity":pack["pack_id"],"source_hash":pack["deterministic_sha256"],"access":"READ_ONLY_REFERENCE"},);seen={};inspected={}
 for x in old.candidates:duplicate_record(ProductionQuestionCandidateV1(**x),seen)
 def validator(c,d,a):r=strength_validator(c,d,a);inspected[c.candidate_id]=(c,d,r);return r
 bank,summary=produce_course_bank("STRENGTH_OF_MATERIALS",pack["pack_id"],pack["deterministic_sha256"],evidence,families,reviewer=lambda s,l:reviewer(families,inspected,s,l),duplicate_analyzer=lambda c,_:duplicate_record(c,seen),validator=validator);return replace(bank,bank_id="bank:STRENGTH_OF_MATERIALS:checkpoint-200:v1"),summary
