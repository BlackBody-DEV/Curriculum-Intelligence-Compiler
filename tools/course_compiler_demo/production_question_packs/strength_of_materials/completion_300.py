...
def _completion_family(i,course):
 ...
 def params(n,builder=base.parameter_builder):
  x=builder(n);x["a"]+=400;x["b"]+=20;x["pressure"]+=2;x["radius"]+=400;x["thickness"]+=20;x["length"]+=4000;x["modulus"]+=40;x["inertia"]+=40000;x["area"]+=400;return x
 return replace(base,parameter_builder=params)
def build_completion_bank():
 prior=(build_bank()[0],build_checkpoint_bank()[0]);pack=build_physics_engineering_course_catalog();validate_physics_engineering_course_catalog(pack);course=pack["courses"]["STRENGTH_OF_MATERIALS"];families=tuple(_completion_family(i,course) for i in range(10));evidence=({"evidence_id":"STRENGTH_OF_MATERIALS:COURSE_CATALOG","source_identity":pack["pack_id"],"source_hash":pack["deterministic_sha256"],"access":"READ_ONLY_REFERENCE"},);seen={};inspected={}
 for b in prior:
 ...
 bank,summary=produce_course_bank("STRENGTH_OF_MATERIALS",pack["pack_id"],pack["deterministic_sha256"],evidence,families,reviewer=lambda s,l:reviewer(families,inspected,s,l),duplicate_analyzer=lambda c,_:duplicate_record(c,seen),validator=validator);return replace(bank,bank_id="bank:STRENGTH_OF_MATERIALS:completion-300:v1"),summary
