...
def build_checkpoint_bank():
 old=build_bank()[0];pack=build_physics_engineering_course_catalog();fs=tuple(_f(i,pack["courses"]["FLUID_DYNAMICS"]) for i in range(10));seen={};[duplicate_record(ProductionQuestionCandidateV1(**x),seen) for x in old.candidates];ev=({"evidence_id":"FLUID_DYNAMICS:CATALOG","source_identity":pack["pack_id"],"source_hash":pack["deterministic_sha256"]},);b,s=produce_course_bank("FLUID_DYNAMICS",pack["pack_id"],pack["deterministic_sha256"],ev,fs,duplicate_analyzer=lambda c,_:duplicate_record(c,seen),validator=validator);return replace(b,bank_id="bank:FLUID_DYNAMICS:checkpoint-200:v1"),s
