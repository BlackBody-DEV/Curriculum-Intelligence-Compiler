"""Math 115 Pre-Algebra checkpoint: 100 questions taking the course to 200."""
    ...
    for source in sorted(accepted, key=lambda item: item["recipe"].recipe_id):
        ...
        for variant in range(FIRST_NEW_VARIANT, LAST_NEW_VARIANT + 1):
            context = GenerationContextV1(binding, source["topic"]["title"], source["skill"]["title"],
                tuple(source["procedure"]["steps"]), f"math115:{COURSE_ID}:{binding.family_id}", variant)
            ...
            question_id = f"production-question:{COURSE_ID.lower()}:math115:{recipe.recipe_id.rsplit(':', 1)[-1].lower()}:{variant:02d}"
