"""Math 118 Pre-Algebra completion: 100 questions taking the course to 300."""
...
from .bank import _prompt_fingerprint, build_checkpoint_bank
...
def build_completion_300_bank() -> tuple[tuple[dict[str, Any], ...], dict[str, Any]]:
    ...
    for source in sorted(accepted, key=lambda item: item["recipe"].recipe_id):
        ...
        for variant in range(FIRST_NEW_VARIANT, LAST_NEW_VARIANT + 1):
            context = GenerationContextV1(binding, source["topic"]["title"], source["skill"]["title"],
                tuple(source["procedure"]["steps"]), f"math118:{COURSE_ID}:{binding.family_id}", variant)
            ...
            question_id = f"production-question:{COURSE_ID.lower()}:math118:{recipe.recipe_id.rsplit(':', 1)[-1].lower()}:{variant:02d}"
            ...
            rows.append({"question_id": question_id, "course_id": COURSE_ID,
                ...
                "difficulty": difficulties[(variant-FIRST_NEW_VARIANT)%3], "prompt": result.prompt,
    ...
    return tuple(rows), {"course_id":COURSE_ID,"before":200,"added":100,"after":300,
